...
// The offsets follow the additional-info length the way the pointer constructor does. Taking them from
// a fixed header size only holds for a frame without additional info; on a copy of a frame that carries
// some -- a cEMI client may send one -- every PDU view and _ctrl1 pointed that many octets too early.
      ...
      _apdu(_data + other._data[1] + APDU_LPDU_DIFF, *this)
{
    ...
    // A frame longer than the buffer cannot be represented. It is reported as oversized and empty, which
    // valid() rejects, rather than copied past the end of buffer[] into the members behind it. An
    // extended telegram off the bus reaches 265 octets, one more than the buffer holds.
...
// Offsets follow the additional-info length, as in the copy constructor above: a frame that carries some
// otherwise gets every PDU view and _ctrl1 that many octets too early. The parameter is taken by value,
// so the copy constructor has already rejected a frame longer than the buffer.
...
void CemiFrame::fillTelegramTP(uint8_t* data)
{
    ...
    // 03_02_02 2.2.2 Figure 28: the TP1 control field of an L_Data frame is "FT 0 r 1 p1 p0 0 0" -- only
    // the frame type, the repeat flag and the priority are free, bits 6, 4, 1 and 0 are fixed. The cEMI
    // control field reuses those bits for the system-broadcast flag (0 on a system broadcast), the
    // acknowledge request and the confirm flag, and a tunnel client controls all of them, so copying the
    // octet unchanged can put a control field on the line that TP1 does not define: the transceiver
    // rejects the frame, nothing reaches the bus and no confirm comes back. TP1 is a closed medium that
    // only knows broadcast communication mode (03_03_03 2.4.2.4.5.7.1), which the network layer already
    // applies to locally originated frames; a frame handed over by a tunnel client bypasses it.
...
bool CemiFrame::valid() const
{
    ...
    // For external frames (_length != 0), bound the addInfoLen-derived index against _length before
    // dereferencing it -- otherwise valid() is itself the out-of-bounds read it is meant to guard.
